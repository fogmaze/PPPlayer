...
class ISEFWINNER_BASE(nn.Module):
    ...
    def __init__(self):
        self.training_t = torch.tensor([t * Constants.CURVE_SHOWING_GAP for t in range(Constants.SIMULATE_TEST_LEN)], device=self.device)
        super().__init__()
    ...
    def fit_one_iteration(self, data, optimizer, loss_fn) :
        ...
        optimizer.zero_grad()
        output = self(l, l_len, r, r_len, self.training_t.repeat(r.shape[0], 1))
